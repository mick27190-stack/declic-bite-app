...
BEGIN
  ...
  ELSE
    type_label := 'A emporter';
  END IF;

  body_text := type_label || ' - ' || NEW.total_price || ' EUR';

  IF NEW.pickup_time IS NOT NULL AND NEW.pickup_time <> '' THEN
    IF NEW.order_type = 'livraison' THEN
      body_text := body_text || ' - Souhaitee a ' || NEW.pickup_time;
    ELSE
      body_text := body_text || ' - Retrait a ' || NEW.pickup_time;
    END IF;
  END IF;
  ...
  WHERE public.should_receive_site_notification(u.user_id, site_value, 'order')
     OR public.is_order_test_mode_active()
