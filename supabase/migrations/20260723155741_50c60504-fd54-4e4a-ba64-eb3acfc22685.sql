...
ALTER TABLE public.pizza_day_promos
  ...
  CHECK (
    ...
    OR (recurrence = 'monthly' AND week_of_month IN (1,2,3,4,-1))
  );

-- Update pricing function to respect recurrence
    ...
    IF v_category = ANY(pizza_cats) THEN
      ...
      SELECT price INTO promo_price
        ...
        WHERE is_active = true
          ...
          AND (
            recurrence = 'weekly'
            OR (recurrence = 'monthly' AND week_of_month = nth_of_month)
            OR (recurrence = 'monthly' AND week_of_month = -1 AND is_last_of_month)
          )
        ORDER BY
          CASE WHEN recurrence = 'monthly' THEN 0 ELSE 1 END
        LIMIT 1;
