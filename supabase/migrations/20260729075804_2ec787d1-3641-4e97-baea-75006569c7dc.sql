...
BEGIN
  ...
  IF _order_type = 'livraison' THEN
    ...
    -- The earliest deliverable slot never goes below 18h45, even at/after 18h00
    -- when the raw lead time would compute 18h30.
    earliest_allowed := GREATEST(
      CEIL((_paris_minutes + DELIVERY_LEAD)::numeric / 15) * 15,
      DELIVERY_FIRST
    );
...
GRANT EXECUTE ON FUNCTION public.check_order_creation_cutoff(text, text, int) TO anon, authenticated, service_role;