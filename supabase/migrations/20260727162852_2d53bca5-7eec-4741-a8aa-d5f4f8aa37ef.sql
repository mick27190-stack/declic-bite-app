...
-- 1) Ajout du type de promotion
ALTER TABLE public.pizza_day_promos
  ADD COLUMN IF NOT EXISTS promo_type text NOT NULL DEFAULT 'fixed';

ALTER TABLE public.pizza_day_promos
  DROP CONSTRAINT IF EXISTS pizza_day_promos_promo_type_check;
ALTER TABLE public.pizza_day_promos
  ADD CONSTRAINT pizza_day_promos_promo_type_check
  CHECK (promo_type IN ('fixed','second_half','bogo'));

-- price devient nullable (obligatoire uniquement pour 'fixed', vérifié par contrainte)
ALTER TABLE public.pizza_day_promos ALTER COLUMN price DROP NOT NULL;

ALTER TABLE public.pizza_day_promos
  DROP CONSTRAINT IF EXISTS pizza_day_promos_price_required_check;
ALTER TABLE public.pizza_day_promos
  ADD CONSTRAINT pizza_day_promos_price_required_check
  CHECK (promo_type <> 'fixed' OR price IS NOT NULL);

-- 2) Mise à jour du calcul de total : applique les remises par paires
...
DECLARE
  ...
  qty int;
  base_total numeric;       -- prix unitaire "de base" (hors suppléments)
  line_base_total numeric;  -- total base pour la ligne (tient compte des paires)
  supp_total numeric;
...
BEGIN
  ...
  LOOP
    ...
    IF v_category = ANY(pizza_cats) THEN
      ...
      IF promo_type_val = 'second_half' THEN
        pairs := qty / 2;
        singles := qty % 2;
        line_base_total := pairs * (raw_size_price + raw_size_price / 2.0) + singles * raw_size_price;
        base_total := raw_size_price; -- pour suppléments (par unité)
      ELSIF promo_type_val = 'bogo' THEN
        pairs := qty / 2;
        singles := qty % 2;
        line_base_total := (pairs + singles) * raw_size_price;
    ...
    IF line_base_total IS NOT NULL THEN
      total := total + line_base_total + supp_total * qty;
    ELSE
