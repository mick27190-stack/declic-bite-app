...
import { Bell, ShoppingBag, MessageSquare, CheckCheck } from 'lucide-react';
...
function NotificationItem({ notification, onRead }: { notification: Notification; onRead: (id: string) => void }) {
  ...
  const handleClick = () => {
    ...
    // Both chat replies and order tracking live on the customer profile page.
    navigate('/profile');
  };
...
export default function CustomerNotificationBell() {
  ...
  // Only show the bell to signed-in customers.
  if (!user) return null;
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {unreadCount > 0 && (
            <span aria-hidden="true" className="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-primary text-primary-foreground text-[10px] font-bold flex items-center justify-center animate-in zoom-in-50">
