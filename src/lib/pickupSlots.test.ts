...
// Helper: build a Date at a given hour/minute (date itself is irrelevant).
const at = (h: number, m: number) => new Date(2026, 5, 21, h, m, 0, 0);
...
describe("computePickupSlots edge cases", () => {
  ...
  it("1 minute before the 15-min threshold pushes to the next slot", () => {
    ...
    const slots = computePickupSlots(at(19, 1));
  ...
  it("2 minutes before the threshold also blocks the nearest slot", () => {
    ...
    const slots = computePickupSlots(at(19, 13));
    expect(slots).not.toContain("19:15");
    expect(slots[0]).toBe("19:30");
  });

  it("blocks all earlier slots and only keeps reachable ones", () => {
    const slots = computePickupSlots(at(20, 0)); // earliest 20:15
    expect(slots).toEqual(["20:15", "20:30", "20:45", "21:00", "21:15", "21:30", "21:45"]);
  });

  it("right at the last slot's lead window still offers it", () => {
    // 21h30 + 15 = 21h45 exactly -> last slot 21h45 remains.
    const slots = computePickupSlots(at(21, 30));
    expect(slots).toEqual(["21:45"]);
  });

  it("midnight (00h00) returns the full next-service slot list", () => {
    const slots = computePickupSlots(at(0, 0));
